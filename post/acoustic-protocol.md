...
Noise is mostly impossible to eliminate, unless the receiver has a [secondary mic in the opposite direction](https://en.wikipedia.org/wiki/Active_noise_control). This is standard for phones, but not laptops and desktops.
...
## Trivia
BTW, did you know that lower frequencies have more unique phases available? Me neither. This makes sense, as higher frequencies can "exhaust" the sampling-rate, reducing the set of possible phases. I actually lied, I've only simulated this in my brain with a low-frequency wave and a half-sample-rate wave. ~I'm not sure if it's possible to preserve phase information by using a frequency that isn't _perfectly half_ the sample-rate.~ It [seems possible](https://youtu.be/G3CIJ3l5Fd8?lc=Ugxr5tkqLURU2fCVAY14AaABAg) (`alt`: my comment on "EXTREME WAVE FOLDING: a Pure Data experiment." where I mention [Moiré patterns](https://en.wikipedia.org/wiki/Moir%C3%A9_pattern))
